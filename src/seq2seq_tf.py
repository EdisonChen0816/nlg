...
class Seq2seq:
    ...
    def __init__(self, logger, train_path, eval_path, max_len, batch_size, epoch, loss, rate,
                 num_units, num_layers, dropout, tf_config, model_path, summary_path,
                 embedding_dim=300, use_attention=True, use_beam_search=True):
        self.logger = logger
    ...
    def model(self, seq_inputs, seq_inputs_length):
        ...
        with tf.variable_scope("decoder"):
            decoder_embedding = tf.Variable(tf.random_uniform([config.target_vocab_size, self.embedding_dim]),
